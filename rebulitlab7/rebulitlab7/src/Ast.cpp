...
FuncExpr::FuncExpr(SymbolEntry* se, ExprNode* param)
...
void ArrayInitList::addExpr(ExprNode* expr) {
...
void ArrayInitList::fill() {
    Type* type = ((ArrayType*)(this->getType()))->getElementType();
    if (type->isArray()) {
        while (!isFull())
            this->addExpr(new ArrayInitList(new ConstantSymbolEntry(type)));
        ExprNode* temp = expr;
        while (temp) {
            ((ArrayInitList*)temp)->fill();
            temp = (ExprNode*)(temp->getNext());
        }
    }
    if (type->isInt()) {
