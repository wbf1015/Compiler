...
LoadMInstruction::LoadMInstruction(MachineBlock* p,MachineOperand* dst, MachineOperand* src1, MachineOperand* src2,
    int cond)
{
    this->parent = p;
    this->type = MachineInstruction::LOAD;
    this->op = -1;//这个op问了下林坤学长，说是没啥用
    this->cond = cond;
    this->def_list.push_back(dst);
    this->use_list.push_back(src1);
    if (src2)
...
{   
    //相当于fp和lr必pop，剩下的就是调用时保存的寄存器
    this->offset = (parent->getSavedRegs().size() + 2) * 4;//感觉这个可以直接移植？
    if(!inst_list.empty()){
        fprintf(yyout, ".L%d:\n", getno());
        for(long unsigned int i=0;i<inst_list.size();i++){
            //这条指令会帮助我们pop所有calle saved
            if(getMI(i)->isSpecialBranch()){
                MachineOperand* op = getReg(11);//拿到fp
                MachineOperand* op2 = getReg(14);//拿到lr
                MachineInstruction* mi = new BigParaStackMInstrcution(this,StackMInstrcuton::POP,parent->getSavedRegs(),op,op2);
                mi->output();
            }
            //如果这个函数参数大于四我就以r3为基准，向上挨个取后面的参数
            if(parent->getparaSize()>4&&getMI(i)->isStore()){
                MachineOperand* op = getMI(i)->getUse()[0];
                if(op->isReg()&&op->getReg()==3){
                    if(first){first=false;}
                    else{
                        MachineOperand* mo = getReg(11);//拿到fp
                        MachineOperand* mo2 = getReg(3);//拿到三号寄存器
                        MachineOperand* mo3 = getImm(offset);
                        changeoffset(4);
                        MachineInstruction* mi = new LoadMInstruction(this,mo2,mo,mo3);
                        mi->output();
                    }
                }
            }
            //函数末尾的指令
            if(getMI(i)->isSpecialADD()){
                MachineOperand* mo = getDefmo(getMI(i),0);
                MachineOperand* mo2 = getUsemo(getMI(i),0);
                //判断是不是add sp sp #4这种调整栈结构
                if(checkReg13(mo)&&checkReg13(mo2)&&getMI(i+1)->isSpecialBranch()){
                    int size = parent->AllocSpace(0);
                    //调整栈结构 大于常数最高限度就Load到寄存器
                    if(abs(size)>=256){
                        MachineOperand* mo3 = getReg(1);
                        MachineOperand* mo4 = getImm(size);
                        MachineInstruction* mi = new LoadMInstruction(nullptr,mo3,mo4);
                        mi->output();
                        getUsemo(getMI(i),1)->setReg(1);
                    }else{
                        if(abs(size)==256){
                            MachineOperand* moo = getReg(1);
                            int k=moo->getVal();
                            i+=k;i-=k;
                            getUsemo(getMI(i),1)->setVal(size);
                        }else{
                            getUsemo(getMI(i),1)->setVal(size);
                        }
                    }
    ...
    //         //这里就是承接函数返回需要pop恢复哪些CALLee saved
    //         if ((*it)->isSpecialBranch()) {
    //             auto fp = new MachineOperand(MachineOperand::REG, 11);
    //             auto lr = new MachineOperand(MachineOperand::REG, 14);
    //             auto cur_inst =new BigParaStackMInstrcution(this, StackMInstrcuton::POP,parent->getSavedRegs(), fp, lr);
    //             cur_inst->output();
    //         }
    //         if (num > 4 && (*it)->isStore()) {
...
void MachineUnit::PrintGlobalDecl()
{
    // TODO
    // 需要自己写
    //分情况先把记号标注好了 有初始化的常数 没初始化的变量 有初始化的变量
